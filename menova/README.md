# MenoWise — Agentic AI Health Assistant for Menopause
...
MenoWise solves this by applying progressively sophisticated RAG (Retrieval Augmented Generation) to menopause health knowledge, giving women grounded, personalized, safety-conscious answers.

## Architecture

MenoWise uses a **3-tier RAG architecture** where each tier exists because the previous one fails in a specific, predictable way:
...
├── knowledge-base/
│   └── menowise-knowledge-base.txt    ← All 20 documents for ingestion
├── neo4j/
...
4. **Create Pinecone index:** Name: `menowise`, Dimensions: `3072`, Metric: `cosine`
...
This project was built as an AI Product Management portfolio piece, inspired by [Sara Soleymani's RAG series](https://www.linkedin.com/pulse/rag-revenue-teams-from-simple-retrieval-agentic-graph-sara-soleymani-cjuec/) which demonstrated the same tier progression for sales/GTM use cases. MenoWise applies the framework to healthcare — a domain where RAG quality isn't just a product metric, it's a safety requirement.
